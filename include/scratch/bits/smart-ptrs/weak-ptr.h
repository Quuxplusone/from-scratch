...
public:
    ...
    weak_ptr(const shared_ptr<T>& rhs) noexcept : m_ctrl(rhs.m_ctrl) {
        m_ptr = rhs.m_ptr;
        increment_weak_count();
    }

    template<class Y> weak_ptr(const shared_ptr<Y>& rhs) noexcept : m_ctrl(rhs.m_ctrl) {
        m_ptr = rhs.m_ptr;
        increment_weak_count();
    }

    weak_ptr(shared_ptr<T>&& rhs) noexcept {
        m_ptr = std::exchange(rhs.m_ptr, nullptr);
        m_ctrl = std::exchange(rhs.m_ctrl, nullptr);
    }

    template<class Y> weak_ptr(shared_ptr<Y>&& rhs) noexcept {
        m_ptr = std::exchange(rhs.m_ptr, nullptr);
        m_ctrl = std::exchange(rhs.m_ctrl, nullptr);
    }
    ...
    // This aliasing constructor is non-standard.
    template<class Y>
    weak_ptr(const weak_ptr<Y>& rhs, T* ptr) noexcept : m_ctrl(rhs.m_ctrl) {
        m_ptr = ptr;
        increment_weak_count();
    }
    ...
    template<class Y> weak_ptr(weak_ptr<Y>&& rhs) noexcept {
        ...
        m_ctrl = std::exchange(rhs.m_ctrl, nullptr);
                      ...
                      weak_ptr& operator=(const shared_ptr<T>& rhs) noexcept { weak_ptr<T>(rhs).swap(*this); return *this; }
    template<class Y> weak_ptr& operator=(const shared_ptr<Y>& rhs) noexcept { weak_ptr<T>(rhs).swap(*this); return *this; }
                      weak_ptr& operator=(shared_ptr<T>&& rhs) noexcept      { weak_ptr<T>(std::move(rhs)).swap(*this); return *this; }
    template<class Y> weak_ptr& operator=(shared_ptr<Y>&& rhs) noexcept      { weak_ptr<T>(std::move(rhs)).swap(*this); return *this; }
