...
template<class T, class Hash, class KeyEq, class Alloc = scratch::allocator<T>>
...
template<class T, class H, class K>
void swap(robin_hood_set<T, H, K>& lhs, robin_hood_set<T, H, K>& rhs) noexcept(noexcept(lhs.swap(rhs))) {
    lhs.swap(rhs);
}
