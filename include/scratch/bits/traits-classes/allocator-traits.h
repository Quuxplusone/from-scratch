...
template<class Alloc> using allocator_value_type_t = decltype(allocator_value_type<Alloc>(priority_tag<1>{}));

template<class Alloc> auto allocator_pointer(priority_tag<1>) -> typename Alloc::pointer;
template<class Alloc> auto allocator_pointer(priority_tag<0>) -> decltype(declval<Alloc&>().allocate(0));
...
private:
    ...
    template<class Alloca, class... Args>
    static auto construct_impl(priority_tag<1>, Alloca& a, value_type *p, Args&&... args) -> decltype(a.construct(p, std::forward<Args>(args)...))
    { return a.construct(p, std::forward<Args>(args)...); }

    template<class... Args>
    static void construct_impl(priority_tag<0>, Alloc&, value_type *p, Args&&... args)
    { ::new (static_cast<void*>(p)) value_type(std::forward<Args>(args)...); }

    template<class Alloca>
    static auto default_construct_impl(priority_tag<1>, Alloca& a, value_type *p) -> decltype(a.default_construct(p))
    { return a.default_construct(p); }

    static void default_construct_impl(priority_tag<0>, Alloc&, value_type *p)
    { ::new (static_cast<void*>(p)) value_type; }

    template<class Alloca>
    static auto destroy_impl(priority_tag<1>, Alloca& a, value_type *p) -> decltype(a.destroy(p))
    { return a.destroy(p); }

    static void destroy_impl(priority_tag<0>, Alloc&, value_type *p)
    { p->~value_type(); }
...
public:
    ...
    static void default_construct(Alloc& a, value_type *p) {
